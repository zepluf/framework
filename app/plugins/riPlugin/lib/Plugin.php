...
class Plugin{
     ...
     * @var
     */
    private static $framework_settings;
	...
	public static function init(){
        ...
        //self::$container->get('event_dispatcher')->dispatch(PluginEvents::onInitEnd, new PluginEvent());
	}
    ...
    public function loadPluginsServices($container){
        ...
        self::get('settings')->load('framework', __DIR__ . '/../../../config/');
        self::$framework_settings = self::get('settings')->get('framework');

        // a hack for zen
        if(self::isAdmin()){
            if(is_array(self::$framework_settings['backend']['preload'])) {
                Plugin::loadPluginServices($container, self::$framework_settings['backend']['preload']);
            }
        }
        else{
            if(is_array(self::$framework_settings['frontend']['preload'])) {
                Plugin::loadPluginServices($container, self::$framework_settings['frontend']['preload']);
            }
        }
    }
     ...
     * @param $plugins
     */
    public static function loadPluginServices($container, $plugins){
                ...
                if(!$loaded_services){
                    foreach (glob($plugin_path."/lib/*.php") as $file) {
                        ...
                        if(ctype_upper($filename[0]))
                            $container->register($plugin.'.'.$filename, 'plugins\\'.$plugin.'\\'.$filename);
                    }
                }
            }
        }
    ...
    public static function loadPlugins($container){
        ...
        // a hack for zen
        if(self::isAdmin()){
            if(is_array(self::$framework_settings['backend']['preload'])) Plugin::loadPlugin(self::$framework_settings['backend']['preload']);
        }
        else{
            if(is_array(self::$framework_settings['frontend']['preload'])) Plugin::loadPlugin(self::$framework_settings['frontend']['preload']);
            // load theme settings
    ...
    public static function loadPlugin($plugins){
        ...
        foreach ($plugins as $plugin){
            if(!in_array($plugin, self::$loaded)){
                ...
                //self::$container->get('event_dispatcher')->dispatch('test', new \Symfony\Component\EventDispatcher\Event());
                ...
                if(!empty($settings)){
					...
					if(isset($settings['routes'])){
						...
						foreach($settings['routes'] as $key => $route){
                            ...
                            self::get('router')->getRouteCollection()->add($plugin_lc_name . '_' . $key, new Route($route['pattern'], $route['defaults'], $route['requirements'], $route['options']));
						}
					}
                ...
                if(Plugin::get($plugin_name) !== false){
                    Plugin::get($plugin_name)->init(self::getContainer());
                }

                // set the dispatcher
                $event = new PluginEvent();

                self::$container->get('event_dispatcher')->dispatch(PluginEvents::onPluginLoadEnd, $event->setPlugin($plugin)->setSettings($settings));
