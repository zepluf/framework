    ...
    public function setPathPatterns($loader) {
        ...
        if(IS_ADMIN_FLAG){
            $loader->setPathPatterns(array(
                DIR_FS_ADMIN . 'includes/templates/template_default/plugins/%plugin%/content/views/%path%/%name%.%format%.%engine%',
                DIR_FS_CATALOG . 'zepluf/app/plugins/%plugin%/content/views/%path%/%name%.%format%.%engine%',
            ));
        }
        else{
            $loader->setPathPatterns(array(
                DIR_FS_CATALOG . DIR_WS_TEMPLATE . 'plugins/%plugin%/content/views/%path%/%name%.%format%.%engine%',
                DIR_FS_CATALOG . 'zepluf/app/plugins/%plugin%/content/views/%path%/%name%.%format%.%engine%',
...
//        if(strpos($view_path, '.') === false) $view_path .= '.php';
    ...
    private function findPathPatterns($view){
        ...
        if(!empty($view[1])){
            $this->addPathPattern('template', $this->patterns['template'] . 'plugins/%plugin%/content/views/%path%/%name%.%format%.%engine%', $patterns);
            $this->addPathPattern($view[0], __DIR__ . '/../../%plugin%/content/views/%path%/%name%.%format%.%engine%', $patterns);
        }
        else {
